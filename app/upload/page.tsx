...
export default function UploadPage() {
  ...
  function acceptFile(f: File) {
    ...
    const ok =
      ...
      f.type === 'image/svg+xml' || f.type === 'application/pdf' ||
      /\.(png|jpe?g|svg|pdf)$/i.test(f.name);
    if (!ok) {
      setError('Unsupported type. Use PNG, JPG, SVG, or PDF.');
      return;
    }
    setFile(f);
    if (!title) {
  ...
  async function handleConvert() {
    ...
    try {
      const out = await convertFileToSvg(file, {
        mode,
        threshold,
        title,
        reference,
  ...
  // ── Publish to gallery (calls /api/upload) ──────────────────────
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      <p className="text-gray-500 mb-6 text-sm">
        Drop a JPG, PNG, SVG, or PDF — we&rsquo;ll extract the line art and produce a
        printable coloring sheet with your title and Bible verse.
      </p>
      ...
      <div className="grid md:grid-cols-[1fr_360px] gap-6">
        ...
        <div>
          ...
          {/* Preview */}
          <div
            className="mt-5 border border-gray-200 rounded-xl bg-gray-50 overflow-hidden flex items-center justify-center"
          ...
          >
            {result ? (
              <object
                ...
                aria-label="Converted coloring sheet preview"
              />
            ) : (
              <p className="text-sm text-gray-400">
                {busy ? 'Converting…' : 'Preview will appear here after conversion.'}
              </p>
            )}
          </div>
        ...
        <aside className="flex flex-col gap-4">
          ...
          <fieldset className="flex flex-col gap-3 border-t border-gray-100 pt-4">
            ...
            <p className="text-xs text-gray-500">
              <strong>Lines</strong> traces edges only (smallest file).{' '}
              <strong>B&amp;W</strong> keeps black areas filled.{' '}
              <strong>Grey</strong> preserves shading.
            </p>
          </fieldset>
          ...
          {published && (
            <div className="text-sm text-green-800 bg-green-50 border border-green-200 rounded-lg p-3">
              ✓ {published.mocked
                  ? 'Saved locally (Supabase not configured). Slug: '
                  : 'Published. View at '}
              {published.mocked
